...
import (
	...
	plugin2 "github.com/khorevaa/r2gitsync/pkg/plugin"
...
type PluginStorage interface {
	...
	List() ([]metadata2.PluginMetadata, error)

	Clear() error
	Registry(m plugin2.Manager) error
...
type FilePluginStorage struct {
	mu  sync.Mutex
	Dir string
	PkgIdx map[string]metadata2.PkgMetadata
	PluginsIdx map[string][]metadata2.PluginMetadata
...
func NewFilePluginStorage(dir string) *FilePluginStorage {
	...
	return &FilePluginStorage{
		...
		PkgIdx: make(map[string]metadata2.PkgMetadata),
		PluginsIdx: make(map[string][]metadata2.PluginMetadata),
	}
}
...
func (s *FilePluginStorage) Registry(mng plugin2.Manager) error {

	err := s.Load()
	if err != nil {
		return err
	}
	var pl []plugin2.Symbol
...
func loadPluginFile(filename string) (pm.Symbol, error) {
	...
	pSymbol, err = pluginFile.Lookup(plugin2.SymbolName)

	return pSymbol, err

}

func loadPkgPlugins(filename string) (pl []plugin2.Symbol, err error) {
...
func (pkg pkgMetadata) getPluginMetadata(p...plugin2.Symbol) (pm []pluginMetadata) {
