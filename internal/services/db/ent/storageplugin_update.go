...
func (spu *StoragePluginUpdate) check() error {
	if _, ok := spu.mutation.StorageID(); spu.mutation.StorageCleared() && !ok {
		return errors.New(`ent: clearing a required unique edge "StoragePlugin.storage"`)
	}
	if _, ok := spu.mutation.PluginID(); spu.mutation.PluginCleared() && !ok {
		return errors.New(`ent: clearing a required unique edge "StoragePlugin.plugin"`)
	}
	return nil
}
...
func (spuo *StoragePluginUpdateOne) sqlSave(ctx context.Context) (_node *StoragePlugin, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "StoragePlugin.id" for update`)}
	}
