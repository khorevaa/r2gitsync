...
func (spc *StoragePluginCreate) check() error {
	if _, ok := spc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "StoragePlugin.created_at"`)}
	}
	if _, ok := spc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent: missing required field "StoragePlugin.updated_at"`)}
	}
	if _, ok := spc.mutation.Disable(); !ok {
		return &ValidationError{Name: "disable", err: errors.New(`ent: missing required field "StoragePlugin.disable"`)}
	}
	if _, ok := spc.mutation.StorageID(); !ok {
		return &ValidationError{Name: "storage", err: errors.New(`ent: missing required edge "StoragePlugin.storage"`)}
	}
	if _, ok := spc.mutation.PluginID(); !ok {
		return &ValidationError{Name: "plugin", err: errors.New(`ent: missing required edge "StoragePlugin.plugin"`)}
	}
	return nil
}
