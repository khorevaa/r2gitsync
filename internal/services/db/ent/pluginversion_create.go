...
func (pvc *PluginVersionCreate) check() error {
	...
	if _, ok := pvc.mutation.Description(); !ok {
		return &ValidationError{Name: "description", err: errors.New(`ent: missing required field "PluginVersion.description"`)}
	}
	if v, ok := pvc.mutation.Description(); ok {
		if err := pluginversion.DescriptionValidator(v); err != nil {
			return &ValidationError{Name: "description", err: fmt.Errorf(`ent: validator failed for field "PluginVersion.description": %w`, err)}
		}
	}
	if _, ok := pvc.mutation.Broken(); !ok {
		return &ValidationError{Name: "broken", err: errors.New(`ent: missing required field "PluginVersion.broken"`)}
	}
