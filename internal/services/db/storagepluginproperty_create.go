...
func (sppc *StoragePluginPropertyCreate) check() error {
	if _, ok := sppc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`db: missing required field "StoragePluginProperty.created_at"`)}
	}
	if _, ok := sppc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`db: missing required field "StoragePluginProperty.updated_at"`)}
	}
	if _, ok := sppc.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`db: missing required field "StoragePluginProperty.name"`)}
	}
	if _, ok := sppc.mutation.Value(); !ok {
		return &ValidationError{Name: "value", err: errors.New(`db: missing required field "StoragePluginProperty.value"`)}
	}
	if _, ok := sppc.mutation.PluginID(); !ok {
		return &ValidationError{Name: "plugin", err: errors.New(`db: missing required edge "StoragePluginProperty.plugin"`)}
	}
	return nil
}
