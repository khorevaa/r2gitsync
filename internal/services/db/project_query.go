...
type ProjectQuery struct {
	...
	withMasterStorage  *StorageQuery
	withDevelopStorage *StorageQuery
	withFKs            bool
...
func (pq *ProjectQuery) QueryStorages() *StorageQuery {
	query := &StorageQuery{config: pq.config}
...
// QueryMasterStorage chains the current query on the "master_storage" edge.
func (pq *ProjectQuery) QueryMasterStorage() *StorageQuery {
	query := &StorageQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryDevelopStorage chains the current query on the "develop_storage" edge.
func (pq *ProjectQuery) QueryDevelopStorage() *StorageQuery {
	query := &StorageQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withStorages:       pq.withStorages.Clone(),
		withMasterStorage:  pq.withMasterStorage.Clone(),
		withDevelopStorage: pq.withDevelopStorage.Clone(),
...
func (pq *ProjectQuery) WithStorages(opts ...func(*StorageQuery)) *ProjectQuery {
	query := &StorageQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withStorages = query
	return pq
}

// WithMasterStorage tells the query-builder to eager-load the nodes that are connected to
// the "master_storage" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProjectQuery) WithMasterStorage(opts ...func(*StorageQuery)) *ProjectQuery {
	query := &StorageQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withMasterStorage = query
	return pq
}

// WithDevelopStorage tells the query-builder to eager-load the nodes that are connected to
// the "develop_storage" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProjectQuery) WithDevelopStorage(opts ...func(*StorageQuery)) *ProjectQuery {
	query := &StorageQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withDevelopStorage = query
	return pq
}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context) ([]*Project, error) {
	var (
		...
		loadedTypes = [3]bool{
			pq.withStorages != nil,
			pq.withMasterStorage != nil,
			pq.withDevelopStorage != nil,
		}
	)
	if pq.withMasterStorage != nil || pq.withDevelopStorage != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withStorages; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*Project)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Storages = []*Storage{}
		}
		query.withFKs = true
		query.Where(predicate.Storage(func(s *sql.Selector) {
			s.Where(sql.InValues(project.StoragesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.project_storages
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "project_storages" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "project_storages" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Storages = append(node.Edges.Storages, n)
		}
	}

	if query := pq.withMasterStorage; query != nil {
		ids := make([]uuid.UUID, 0, len(nodes))
		nodeids := make(map[uuid.UUID][]*Project)
		for i := range nodes {
			if nodes[i].project_master_storage == nil {
				continue
			}
			fk := *nodes[i].project_master_storage
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "project_master_storage" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.MasterStorage = n
			}
		}
	}

	if query := pq.withDevelopStorage; query != nil {
		ids := make([]uuid.UUID, 0, len(nodes))
		nodeids := make(map[uuid.UUID][]*Project)
		for i := range nodes {
			if nodes[i].project_develop_storage == nil {
				continue
			}
			fk := *nodes[i].project_develop_storage
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "project_develop_storage" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.DevelopStorage = n
			}
		}
	}
...
func (pgb *ProjectGroupBy) Strings(ctx context.Context) ([]string, error) {
	if len(pgb.fields) > 1 {
		return nil, errors.New("db: ProjectGroupBy.Strings is not achievable when grouping more than 1 field")
	}
	var v []string
	if err := pgb.Scan(ctx, &v); err != nil {
		return nil, err
...
func (pgb *ProjectGroupBy) String(ctx context.Context) (_ string, err error) {
	...
	case 0:
		err = &NotFoundError{project.Label}
	default:
		err = fmt.Errorf("db: ProjectGroupBy.Strings returned %d results when one was expected", len(v))
	}
	return
}
...
func (pgb *ProjectGroupBy) Ints(ctx context.Context) ([]int, error) {
	if len(pgb.fields) > 1 {
		return nil, errors.New("db: ProjectGroupBy.Ints is not achievable when grouping more than 1 field")
	}
	var v []int
	if err := pgb.Scan(ctx, &v); err != nil {
		return nil, err
...
func (pgb *ProjectGroupBy) Int(ctx context.Context) (_ int, err error) {
	...
	case 0:
		err = &NotFoundError{project.Label}
	default:
		err = fmt.Errorf("db: ProjectGroupBy.Ints returned %d results when one was expected", len(v))
	}
	return
}
...
func (pgb *ProjectGroupBy) Float64s(ctx context.Context) ([]float64, error) {
	if len(pgb.fields) > 1 {
		return nil, errors.New("db: ProjectGroupBy.Float64s is not achievable when grouping more than 1 field")
	}
	var v []float64
	if err := pgb.Scan(ctx, &v); err != nil {
		return nil, err
...
func (pgb *ProjectGroupBy) Float64(ctx context.Context) (_ float64, err error) {
	...
	case 0:
		err = &NotFoundError{project.Label}
	default:
		err = fmt.Errorf("db: ProjectGroupBy.Float64s returned %d results when one was expected", len(v))
	}
	return
}
...
func (pgb *ProjectGroupBy) Bools(ctx context.Context) ([]bool, error) {
	if len(pgb.fields) > 1 {
		return nil, errors.New("db: ProjectGroupBy.Bools is not achievable when grouping more than 1 field")
	}
	var v []bool
	if err := pgb.Scan(ctx, &v); err != nil {
		return nil, err
...
func (pgb *ProjectGroupBy) Bool(ctx context.Context) (_ bool, err error) {
	...
	case 0:
		err = &NotFoundError{project.Label}
	default:
		err = fmt.Errorf("db: ProjectGroupBy.Bools returned %d results when one was expected", len(v))
	}
	return
}
...
func (ps *ProjectSelect) Strings(ctx context.Context) ([]string, error) {
	if len(ps.fields) > 1 {
		return nil, errors.New("db: ProjectSelect.Strings is not achievable when selecting more than 1 field")
	}
	var v []string
	if err := ps.Scan(ctx, &v); err != nil {
		return nil, err
...
func (ps *ProjectSelect) String(ctx context.Context) (_ string, err error) {
	...
	case 0:
		err = &NotFoundError{project.Label}
	default:
		err = fmt.Errorf("db: ProjectSelect.Strings returned %d results when one was expected", len(v))
	}
	return
}
...
func (ps *ProjectSelect) Ints(ctx context.Context) ([]int, error) {
	if len(ps.fields) > 1 {
		return nil, errors.New("db: ProjectSelect.Ints is not achievable when selecting more than 1 field")
	}
	var v []int
	if err := ps.Scan(ctx, &v); err != nil {
		return nil, err
...
func (ps *ProjectSelect) Int(ctx context.Context) (_ int, err error) {
	...
	case 0:
		err = &NotFoundError{project.Label}
	default:
		err = fmt.Errorf("db: ProjectSelect.Ints returned %d results when one was expected", len(v))
	}
	return
}
...
func (ps *ProjectSelect) Float64s(ctx context.Context) ([]float64, error) {
	if len(ps.fields) > 1 {
		return nil, errors.New("db: ProjectSelect.Float64s is not achievable when selecting more than 1 field")
	}
	var v []float64
	if err := ps.Scan(ctx, &v); err != nil {
		return nil, err
...
func (ps *ProjectSelect) Float64(ctx context.Context) (_ float64, err error) {
	...
	case 0:
		err = &NotFoundError{project.Label}
	default:
		err = fmt.Errorf("db: ProjectSelect.Float64s returned %d results when one was expected", len(v))
	}
	return
}
...
func (ps *ProjectSelect) Bools(ctx context.Context) ([]bool, error) {
	if len(ps.fields) > 1 {
		return nil, errors.New("db: ProjectSelect.Bools is not achievable when selecting more than 1 field")
	}
	var v []bool
	if err := ps.Scan(ctx, &v); err != nil {
		return nil, err
...
func (ps *ProjectSelect) Bool(ctx context.Context) (_ bool, err error) {
	...
	case 0:
		err = &NotFoundError{project.Label}
	default:
		err = fmt.Errorf("db: ProjectSelect.Bools returned %d results when one was expected", len(v))
	}
	return
}
